...
def train_model(train_num,val_num,num_rounds,num_epochs,depth,diff,wdir):
    ...
    check = make_checkpoint(wdir+'speck32_M_chen2'+'_best_r'+str(num_rounds)+'.h5');
    ...
    pickle.dump(history.history, open(wdir+'speck32_M_chen2'+'r'+str(num_rounds)+'.p', 'wb'));
    np.save(wdir+'speck32_M_chen2'+'r'+str(num_rounds)+'val_acc'+'.npy', history.history['val_acc']);
    np.save(wdir+'speck32_M_chen2'+'r'+str(num_rounds)+'val_loss'+'.npy', history.history['val_loss']);
    np.save(wdir+'speck32_M_chen2'+'r'+str(num_rounds)+'loss'+'.npy', history.history['loss']);
    np.save(wdir+'speck32_M_chen2'+'r'+str(num_rounds)+'acc'+'.npy', history.history['acc']);
    np.save(wdir + 'speck32_M_chen2_ksback_' + 'r' + str(num_rounds) + '.npy', ks_back)
    print("Best validation accuracy: ", np.max(history.history['val_acc']));
    model.save(wdir+'speck32_M_chen2'+'r'+str(num_rounds) + '.h5')