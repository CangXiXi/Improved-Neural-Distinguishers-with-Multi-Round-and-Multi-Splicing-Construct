...
def train_model(train_num,val_num,num_rounds,num_epochs,depth,diff,wdir):
    ...
    check = make_checkpoint(wdir+'speck32_M_hou2'+'_best_r'+str(num_rounds)+'.h5');
    ...
    pickle.dump(history.history, open(wdir+'speck32_M_hou2'+'r'+str(num_rounds)+'.p', 'wb'));
    np.save(wdir+'speck32_M_hou2'+'r'+str(num_rounds)+'val_acc'+'.npy', history.history['val_acc']);
    np.save(wdir+'speck32_M_hou2'+'r'+str(num_rounds)+'val_loss'+'.npy', history.history['val_loss']);
    np.save(wdir+'speck32_M_hou2'+'r'+str(num_rounds)+'loss'+'.npy', history.history['loss']);
    np.save(wdir+'speck32_M_hou2'+'r'+str(num_rounds)+'acc'+'.npy', history.history['acc']);
    np.save(wdir + 'speck32_M_hou2_ksback_' + 'r' + str(num_rounds) + '.npy', ks_back)
    print("Best validation accuracy: ", np.max(history.history['val_acc']));
    model.save(wdir+'speck32_M_hou2'+'r'+str(num_rounds) + '.h5')

if __name__ == '__main__':
    train_model(train_num=10 ** 6, val_num=10 ** 5, num_rounds=7, num_epochs=100, depth=5,diff=(0x2800, 0x10), wdir='F:\Procedure\Test_back_final\全随机密钥结果检验/')