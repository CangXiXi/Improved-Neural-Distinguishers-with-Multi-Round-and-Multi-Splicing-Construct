...
def expand_key(k,t):
    ...
    for i in range(m,t):
        ...
        subkey[i]=c^Z1[(i-m)%62]^tmp^subkey[i-m]
    return(subkey)
...
def check_testvector():
  ...
  ks = expand_key(key, 36)
  ct = encrypt(pt, ks)
  if (ct == (0x6e06a5, 0xacf156)):
...
def convert_to_binary(arr):
    X = np.zeros((192 * WORD_SIZE(),len(arr[0])),dtype=np.uint8)
    for i in range(192* WORD_SIZE()):
        index = i // WORD_SIZE();
...
def make_train_data(n, nr, diff):
    ...
    for i in range(int(num)):
        ...
        plain1l[Y==0] = np.frombuffer(urandom(4*num_rand_samples),dtype=np.uint32) % pow(2,24)
        plain1r[Y==0] = np.frombuffer(urandom(4*num_rand_samples),dtype=np.uint32) % pow(2,24)
        ...
        X += [ctdatal,ctdatar,ctdatal2, ctdatar2]
        i += 1
